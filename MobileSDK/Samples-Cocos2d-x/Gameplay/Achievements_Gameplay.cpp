    ...
    _In_ bool hasNextPage)
{
    ...
    if (SUCCEEDED(result))
    {
        ...
        GameScene::getInstance()->m_achievementsLayer->setHasNextResultsPage(hasNextPage);

        if (hasNextPage)
        {
            // Store Achievement Result Handle
            GameScene::getInstance()->m_achievementsLayer->setAchievementsResultHandle(achievementsResultHandle);
        }
        else
        {
            GameScene::getInstance()->m_achievementsLayer->setAchievementsResultHandle(nullptr);
        }
    }
}
