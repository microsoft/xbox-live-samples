    ...
    _In_ bool hasNextPage)
{
    ...
    if (SUCCEEDED(result))
    {
        ...
        AchievementsLayer_SetHasNextPage(hasNextPage);

        if (hasNextPage)
        {
            // Store Achievement Result Handle
            AchievementsLayer_SetAchievementsResultHandle(achievementsResultHandle);
        }
        else
        {
            AchievementsLayer_SetAchievementsResultHandle(nullptr);
        }
    }
}
