...
namespace Utils
{
    ...
    {
        Microsoft::WRL::ComPtr<IInspectable> tempb(reinterpret_cast<IInspectable*>(inbuf));
    ...
    Platform::String^ GetDebugNameForLocalConsole()
    {
        ...
        auto user = Sample::Instance()->GetCurrentUser();

        if (user != nullptr)
        {
            debugName = user->gamertag();
            debugName += L"'s console";
        }

        return ref new Platform::String(debugName.c_str());
    }
}
...
void ChatIntegrationLayer::Initialize()
{
    ...
    m_chatOverlay = Windows::Gaming::UI::GameChatOverlay::GetDefault();
    m_chatOverlay->DesiredPosition = Windows::Gaming::UI::GameChatOverlayPosition::MiddleRight;
#endif
...
void ChatIntegrationLayer::OnSecureDeviceAssocationConnectionEstablished(
    __in Microsoft::Xbox::Samples::NetworkMesh::MeshConnection^ connection,
    __in std::vector<std::wstring> xuids
)
{
...
void ChatIntegrationLayer::ProcessStateChanges()
{
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        case game_chat_state_change_type::communication_relationship_adjuster_changed:
            Sample::Instance()->LogToConsole("GameChatManager::ProcessStateChanges() communication_relationship_adjuster_changed\n");
            break;

        case game_chat_state_change_type::text_chat_received:
            Sample::Instance()->LogToConsole("GameChatManager::ProcessStateChanges() text_chat_received\n");
            HandleTextMessage(change, false);
            break;

        case game_chat_state_change_type::text_conversion_preference_changed:
            Sample::Instance()->LogToConsole("GameChatManager::ProcessStateChanges() text_conversion_preference_changed\n");
            break;

        case game_chat_state_change_type::transcribed_chat_received:
            Sample::Instance()->LogToConsole("GameChatManager::ProcessStateChanges() transcribed_chat_received\n");
...
void ChatIntegrationLayer::ProcessDataFrames()
{
    ...
    if (dataFrameCount > 0)
    {
        ...
        for (uint32_t dataFrameIndex = 0; dataFrameIndex < dataFrameCount; ++dataFrameIndex)
        {
            ...
            Microsoft::WRL::ComPtr<ABI::Windows::Storage::Streams::IBuffer> wrapBuffer = Make<WrapBuffer>(dataFrame->packet_buffer, dataFrame->packet_byte_count);
    ...
    )
{
    ...
    name = ref new Platform::String(Sample::Instance()->GetNameFromXuid(xuid->Data()).c_str());
...
#if defined(NTDDI_WIN10_RS2) && (NTDDI_VERSION >= NTDDI_WIN10_RS2)
    m_chatOverlay->AddMessage(
        name,
        message,
        transcribed ?
            Windows::Gaming::UI::GameChatMessageOrigin::Voice :
            Windows::Gaming::UI::GameChatMessageOrigin::Text
    );
#endif
}
...
{
    auto localUser = GetChatUserByXboxUserId(Sample::Instance()->GetCurrentUser()->xbox_user_id().c_str());
...
chat_user*
ChatIntegrationLayer::GetChatUserByXboxUserId(PCWSTR xuid)
{
    if (m_initialized)
    {
        uint32_t count;
...
std::vector<std::wstring> ChatIntegrationLayer::GetChatUsersXuids()
{
