...
namespace
{
    ...
    const int c_findMatchBtnId          = 3203;
...
void Sample::SetupUI()
{
    ...
    m_findMatchBtn = dynamic_cast<ATG::ImageButton*>(m_sessionDetailsPanel->Find(3203));
    ...
    {
        if (m_findMatchBtn->GetImageId() == c_findMatchBtnId)
        {
            FindMatch();
            m_findMatchBtn->SetImageId(c_exitGameBtnId);
        }
        else if (m_findMatchBtn->GetImageId() == c_exitGameBtnId)
        {
            if (m_multiplayerManager->match_status() != match_status::none)
            {
                CancelMatch();
            }
            else if (!m_isLeavingGame && m_multiplayerManager->game_session() != nullptr)
            {
            ...
            m_findMatchBtn->SetImageId(c_findMatchBtnId);
        }
    });
...
void Sample::CreateDeviceDependentResources()
{
    ...
    m_joinGameImage = std::make_unique<DX::Texture>(m_deviceResources->GetD3DDevice(), L"Assets//btnFind.png");
    m_exitGameImage = std::make_unique<DX::Texture>(m_deviceResources->GetD3DDevice(), L"Assets//btnExitON.png");
    m_ui->RegisterImage(c_findMatchBtnId, m_joinGameImage->Get());
