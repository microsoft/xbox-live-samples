    ...
    if (pad.IsConnected())
    {
        ...
        if (!m_ui->Update(elapsedTime, pad))
        {
            ...
            else if (m_appState == APPSTATE::APP_IN_GAME)
            {
                ...
                {
                    if (!m_isFindMatchInProgress)
                    {
                        m_isFindMatchInProgress = true;
                        FindMatch();
                    }
                }
                ...
                if (m_gamePadButtons.b == GamePad::ButtonStateTracker::ButtonState::PRESSED)
                {
                    ...
                    else if (m_multiplayerManager->game_session() != nullptr && !m_isLeavingGame )
                    {
                        m_isLeavingGame = true;
                        LeaveGameSession();
                    }
                    else if(!m_isFindMatchInProgress && !m_isLeavingGame && m_multiplayerManager->game_session() == nullptr)
                    {
                        RemoveLocalUser();
                    }
                }
