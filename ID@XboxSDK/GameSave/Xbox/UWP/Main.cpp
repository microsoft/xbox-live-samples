...
public:
    ...
    virtual void SetWindow(CoreWindow^ window)
    {
        ...
        window->KeyUp +=
            ref new TypedEventHandler<CoreWindow^, KeyEventArgs^>(this, &ViewProvider::OnKeyUp);
...
protected:
    ...
    {
        Log::WriteAndDisplay("OnActivated(%ws)\n", args->Kind.ToString()->Data());

        auto argsWithUser = dynamic_cast<IActivatedEventArgsWithUser^>(args);
        if (argsWithUser && argsWithUser->User)
        {
            Log::WriteAndDisplay("Activating user ID: %ws\n", argsWithUser->User->NonRoamableId->Data());
        }
    ...
    {
        m_sample->OnKeyUp(args);
    }

    void OnWindowActivated(CoreWindow^ sender, WindowActivatedEventArgs^ args)
    {
        Log::WriteAndDisplay("Window OnActivated(%ws)\n", args->WindowActivationState.ToString()->Data());
    }
    ...
    void OnVisibilityChanged(CoreWindow^ sender, VisibilityChangedEventArgs^ args)
    {
        ...
        if (m_visible)
            m_sample->OnWindowActivated();
        else
            m_sample->OnWindowDeactivated();
    }
...
private:
    ...
    void HandleWindowSizeChanged()
    {
        ...
        m_sample->OnWindowSizeChanged(outputWidth, outputHeight, rotation, m_DPI);
    }
};
